    ...
    public class StorageController : CmsControllerBase
    {
        ...
        public Task<IActionResult> RenameFolder(RenameFolderRequest Request)
        {
            return TryCatchLogAsync(async () =>
            {
                if (ModelState.IsValid)
                {
                    ...
                    var response = new SpatiumResponse()
                    {
                        Message = $"Folder Renamed Successfully from {Request.OldName} To {folder.Name} ",
                        Success = true
                    };
                    return Ok(response);
        ...
        public Task<IActionResult> CreateFile(AddFileRequest FileRequest)
        {
            return TryCatchLogAsync(async () =>
            {
                if (ModelState.IsValid)
                {
                    ...
                    //check the capacity of the storage to upload new file
                    var storageCapacity = unitOfWork.StorageRepository.GetBlogFilesCapcity(blogId);
                    var subscription=await unitOfWork.SubscriptionRepository.GetSubscriptionByIdAsync(blog.SubscriptionId);
                    var FileSizeByMb = ( double.Parse(filesize)) / 1024 / 1024;
                    if (subscription.StorageCapacity <= storageCapacity + FileSizeByMb) throw new SpatiumException("Sorry You Are Reached The Limitation Of Storage Capacity");
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (OldFile != null)
                    {
                        ...
                        if (Request.File is not null)
                        {
                            ...
                            var storageCapacity = unitOfWork.StorageRepository.GetBlogFilesCapcity(blogId);
                            var subscription = await unitOfWork.SubscriptionRepository.GetSubscriptionByIdAsync(blog.SubscriptionId);
                            var FileSizeByMb = (double.Parse(Request.File.Length.ToString())) / 1024 / 1024;
                            if (subscription.StorageCapacity <= storageCapacity + FileSizeByMb) throw new SpatiumException("Sorry You Are Reached The Limitation Of Storage Capacity");
        ...
        public Task<IActionResult> MoveFilesBulk(List<int> filesIds, int? folderIdDestination = null)
        {
            return TryCatchLogAsync(async () =>
            {
                ...
                return Ok("Files Moved Succussed...");
            });
        }
        ...
        public Task<IActionResult> DeleteFilesBulk(List<int> filesIds)
        {
            return TryCatchLogAsync(async () =>
            {
                ...
                return Ok("Files Deleted Succussed...");
            });
        }
        ...
        public Task<IActionResult> GetAllFiles([FromQuery] GetEntitiyParams entityParams)
        {
            ...
            {
                var blogId = GetBlogId();
                if ((entityParams.StartDate != null && entityParams.EndDate == null) || (entityParams.EndDate != null && entityParams.StartDate == null)) throw new SpatiumException("you should enter both of date start and end date");
                ...
                var response = mapper.Map<List<ViewFile>>(files);
                var total = await unitOfWork.StorageRepository.GetAllFiles(blogId);
                return Ok(new
                {
                    Success = true,
                    ...
                    Data = response,
                    Total= total.Count(),
                    PagesNumber= total.Count() / entityParams.PageSize+1,
                }
                );
