    ...
    public class AutoMapperProfiles
    {
        public class AutoMapperProfile : Profile
        {
            public AutoMapperProfile()
            {
                ...
                CreateMap<ApplicationUser, UserLogInDetailes>()
                    .ForMember(dest => dest.ImageProfilePath, otp => otp.MapFrom<LoginUserResolver>());
                #endregion
                ...
                CreateMap<ExternalPostResponse, Post>().ReverseMap();
                ...
                CreateMap<Subscription, GetAllSubscriptionDto>()
                    .ForMember(dest=>dest.SEOUsage,otp=>otp.MapFrom(src=>src.SEO_Usage)).ReverseMap();
                  
                CreateMap<Subscription,GetYourCurrentPlanDto>().ForMember(dest => dest.SEOUsage, otp => otp.MapFrom(src => src.SEO_Usage)).ReverseMap();

                CreateMap<BillingHistory, BillHistoryResponse>()
                    .ForMember(des=>des.PaymentType,inp => inp.MapFrom(src =>src.PaymentType.Name))
                  //.ForMember(des=>des.Ammount , inp =>inp.MapFrom(src=> $"{(src.Ammount/100):F2}"))
                  .ForMember(des => des.ExpirationDate, inp => inp.MapFrom(src => src.CreationDate.AddMonths(1))).ReverseMap();
                #endregion
            }
        }
    }
